...
class Player:
    ...
    def draw(self, game_display, camera):
        game_display.blit(
            self.spritesheet[self.sprite_index_y * 3 + self.sprite_index_x],
            (self.x, self.y - camera.y),
        )
    ...
    def on_platform(self, platform):
        return platform.rect.collidepoint(
            (self.x, self.y + self.height)
        ) or platform.rect.collidepoint((self.x + self.width, self.y + self.height))
    ...
    def collide_platform(self, platform, index):
        for i in range(0, self.vel_y):
            if pygame.Rect(self.x, self.y - i, self.width, self.height).colliderect(
                platform.rect
            ):
                if platform.rect.collidepoint(
                    (self.x, self.y + self.height - i)
                ) or platform.rect.collidepoint(
